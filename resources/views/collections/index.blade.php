			...
			<div class="row m-0 p-0">
				...
				@foreach($collections as $collection)
					...
					<div class="card text-black bg-white mb-3 explore_card collection_item_block" data-col-id="{{ $collection->id }}" data-col-name="{{ base64_encode($collection->name) }}">
						<div class="card-body">
							<p class="card-title m-0 p-0 card-title font-weight-bold">{{ $collection->name }}</p>
							...
							<div class="card-title m-0 p-0 card-title font-weight-bold mt-3 mb-1">Created by: <a href="{{ URL::to('/') }}/user/{{$collection->user_id}}/view" class="textcolor-blue pl-2" >{{ $username}}</a></div>
