					...
					<div class="form-group">
						...
						<div class="col-sm-9 col-lg-12 controls">
							<div class="input-group">
								...
								<?php if(is_file(base_path() . '/public/uploads/profile/'.$user_id.'/'.$user->profile_image)): ?>
									
									
									<div class="profile_image_croper_block"></div>
									<script>
										window.profile_image = '<?php echo e(URL::to("/")."/uploads/profile/".$user_id."/".$user->profile_image); ?>';
										
									</script>
									<?php if(trim($user->cropped_image_info) != ''): ?>
										<?php
											$cropped_image_info = json_decode($user->cropped_image_info,true);
										?>
										<?php if(json_last_error() === JSON_ERROR_NONE): ?>
											<script>window.cropped_image_info = JSON.parse('<?php echo $user->cropped_image_info; ?>');</script>
										<?php endif; ?>
									<?php endif; ?>
								<?php endif; ?>
								</div>
...
<?php /**PATH F:\XAMPP7.4\htdocs\growyspace 2.0\resources\views/popups/profile_image.blade.php ENDPATH**/ ?>